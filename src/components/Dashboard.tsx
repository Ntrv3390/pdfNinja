...
const Dashboard = () => {
    ...
    const { mutate: deleteFromUploadthing } = trpc.deleteFromUploadthing.useMutation({
        onSuccess: () => {
            //do nothing
        }
    });
                        ...
                        <li key={file.id} className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow transition hover:shadow-lg">
                            <Link href={`/dashboard/${file.id}`} className="flex flex-col gap-2">
                                <div className="pt-6 px-6 flex w-full items-center justify-between space-x-6">
                                    <div className="h-10 w-10 flex-shrink-0 rounded-full bg-gradient-to-r from-yellow-500 to-orange-500"></div>
                            ...
                            <Button size='sm' className="w-full" variant='destructive' onClick={() =>{ 
                                // delete the pdf from uploadthing
                                const url = file.url;
                                const parts = url.split('/'); // Split the URL by slashes
                                const fileid = parts[parts.length - 1];
                                deleteFromUploadthing({id:fileid})
                                // delete the pdf from neon postgres db
                                deleteFile({ id:file.id });
                                // delete vector embedding assoc with the file from pinecone
                            }}>
